...
export default async function FollowingPage({
  ...
  const { data: rows } = await supabase
    .from('follows')
    .select('following_id, created_at, following:users!following_id(id, name, avatar_url, city, state, rating_avg, top_cook_badge)')
      ...
      <main className="flex-1 w-full max-w-lg mx-auto px-4 py-8 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-extrabold" style={{ color: '#1a3a2a' }}>
            {profile.name ? `${profile.name} is Following` : 'Following'}
          </h1>
        </div>
        ...
        {!rows || rows.length === 0 ? (
          ...
          <div className="bg-white rounded-3xl border border-black/[0.05] shadow-sm divide-y divide-gray-50">
            {rows.map(row => {
              const u = row.following as { id: string; name: string | null; avatar_url: string | null; city: string | null; state: string | null; rating_avg: number; top_cook_badge: boolean } | null;
              if (!u) return null;
